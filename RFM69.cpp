...
bool RFM69::initialize( uint16_t preamble, uint32_t frequency, uint32_t baudrate, int8_t power, char * id, uint8_t id_size )
{
	...
	{ REG_OPMODE, RF_OPMODE_SEQUENCER_ON | RF_OPMODE_LISTEN_OFF | RF_OPMODE_STANDBY },
			...
			/* 0x07 */
			{ REG_FRFMSB, (uint8_t)((frequency >> 16) & 0xFF) },
			/* 0x08 */
			{ REG_FRFMID, (uint8_t)((frequency >> 8) & 0xFF) },
			/* 0x09 */
			{ REG_FRFLSB, (uint8_t)((frequency) & 0xFF) },
			...
			/* 0x2C */
			{ REG_PREAMBLEMSB, ( preamble >> 8 ) & 0xFF },
			/* 0x2D */
			{ REG_PREAMBLELSB, preamble & 0xFF },
			/* 0x2E */
			...
			/* 0x30 */
			{ REG_SYNCVALUE2, 0xC3 }, // NETWORK ID
			/* 0x37 */
...
void RFM69::setPowerLevelAdjusted( int8_t powerLevel )
{
	// cut at -18dBm minimum
	if ( powerLevel < -18 )
	{
		powerLevel = -18;
	}
	// cut at +20dBm maximum
	if ( powerLevel > 20 )
	{
		powerLevel = 20;
	}
	// adjust value
	powerLevel += 11; // add 11dBm to satisfy the formula between +5 to +20 dBm, since highpower is enabled
